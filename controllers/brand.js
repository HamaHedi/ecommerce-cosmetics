...
exports.getBrands = AsyncHandler(async (req, res, next) => {
  // Extract the category name from the request header
  const categoryName = req.query.category;
  // Count the total number of brands
  const brandsCount = await Brand.countDocuments();

  // Initialize the query to find brands
  let brandQuery = Brand.find();

  // If a category name is provided in the header, filter brands by that category
  if (categoryName) {
    brandQuery = brandQuery.where("category").equals(categoryName);
  }

  // Execute the query to find brands
  const brands = await brandQuery.exec();

  // Get unique brand names from all products
  const uniqueBrandNames = await Product.distinct("brand");

  // Create an object to store product counts for each brand name
  const brandProductCounts = {};

  // Count the number of products for each unique brand name
